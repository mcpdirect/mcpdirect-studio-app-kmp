...
@Composable
fun ConfigMCPTemplateDialog(
    toolAgent: AIPortToolAgent,
    template: AIPortToolMakerTemplate,
    onConfirmRequest: (type:Int,config:String,inputs:String) -> Unit,
...
//    var templateName by remember { mutableStateOf("${template.name}_template")}
    ...
    LaunchedEffect(null){
        StudioRepository.getMakerTemplateFromStudio(
            ...
            if(it.successful()) it.data?.let{
                val mcpServerConfig = JSON.decodeFromString<MCPServerConfig>(it.config)
                mcpServerConfig.let {
                    it.url?.let {
...
//    }
    ...
    AlertDialog(
        ...
        title = { Column{
            Text("Modify MCP Server Template")
            Text("of ${template.name}", style = MaterialTheme.typography.titleLarge)
        } },
        text = {
            config?.let {
                mCPServerConfig ->
        ...
        confirmButton = {
            Button(
                ...
                onClick = {
                    ...
                    onConfirmRequest(template.type, config,inputs)
                    onDismissRequest()
                }
            ) {
                Text("Submit")
