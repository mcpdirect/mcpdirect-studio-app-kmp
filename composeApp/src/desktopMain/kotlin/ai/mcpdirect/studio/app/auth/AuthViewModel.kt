...
import ai.mcpdirect.studio.app.data.repository.UserRepository
import ai.mcpdirect.studio.app.data.repository.UserRepositoryImpl
...
class AuthViewModel(private val userRepository: UserRepository = UserRepositoryImpl()) : ViewModel(){
    val userInfo = mutableStateOf<AIPortUser?>(null)
    ...
    var currentScreen by mutableStateOf<AuthScreen>(AuthScreen.AuthOption)
        private set
...
//    object Success : UiState()
    data class SuccessWithData(val data: Any) : UiState()
    data class SuccessWithAccount(val user: AIPortUser) : UiState();
    data class SuccessWithAnonymous(val user: AIPortUser) : UiState();
